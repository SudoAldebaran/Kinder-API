...
# doc THIS GENERIC VIEW ALLOWS LISTING (GET) AND CREATING (POST) OBJECTS
# IN THE DATABASE.
class ParentList(generics.ListCreateAPIView):
    # doc DEFINES THE SET OF OBJECTS TO RETURN IN THE VIEW. HERE, ALL PARENT
    # OBJECTS IN THE DATABASE WILL BE RETURNED.
    queryset = Parent.objects.all()
    # doc INDICATES TO THE VIEW TO USE THE ParentSerializer TO CONVERT PARENT
    # MODEL INSTANCES TO JSON AND VICE-VERSA DURING OBJECT CREATION.
...
class ChildList(generics.ListCreateAPIView):
    # doc DEFINES THE SET OF OBJECTS TO RETURN IN THE VIEW. HERE, ALL CHILD
    # OBJECTS IN THE DATABASE WILL BE RETURNED.
    queryset = Child.objects.all()
    # doc INDICATES TO THE VIEW TO USE THE ChildSerializer TO CONVERT CHILD
    # MODEL INSTANCES TO JSON AND VICE-VERSA DURING OBJECT CREATION.
...
# doc THIS GENERIC VIEW ALLOWS LISTING (GET) OF OBJECTS IN THE DATABASE.
class HouseholdList(generics.ListAPIView):
    # doc DEFINES THE SET OF OBJECTS TO RETURN IN THE VIEW. HERE, ALL
    # HOUSEHOLD OBJECTS IN THE DATABASE WILL BE RETURNED.
    queryset = Household.objects.all()
    # doc INDICATES TO THE VIEW TO USE THE HouseholdSerializer TO CONVERT
    # HOUSEHOLD MODEL INSTANCES TO JSON.
...
class InvitationList(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        # doc FILTER BY THE CONNECTED USER
    ...
    def perform_create(self, serializer):
        ...
        if Invitation.objects.filter(
            ...
            raise ValidationError(
                "An invitation has already been sent to this recipient.")

        if Invitation.objects.filter(
                sender=recipient,
                recipient=sender).exists():
            raise ValidationError(
                "You have already received an invitation from this parent.")
...
class InvitationDetail(generics.RetrieveDestroyAPIView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        # doc DELETE THE HOUSEHOLD IF THE DECLINED INVITATION WAS THE ONLY
        # THING LINKING BOTH PARENTS
        if household.parent1 and household.parent2:
            # doc CHECK IF THERE ARE OTHER INVITATIONS LINKING THESE TWO
            # PARENTS
        ...
        return Response(
            {
                'message': 'Invitation declined and household deleted successfully'},
...
class AcceptInvitationView(generics.UpdateAPIView):
    # doc ENSURE THE USER IS AUTHENTICATED
    permission_classes = [IsAuthenticated]
    ...
    def update(self, request, *args, **kwargs):
        # doc GET THE INVITATION BY ITS ID
        # doc GET THE INVITATION ID FROM THE URL
        invitation_id = kwargs.get('pk')
        try:
            invitation = Invitation.objects.get(id=invitation_id)
        except Invitation.DoesNotExist:
